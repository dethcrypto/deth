...
import { bufferToAddress, bufferToHexData } from '@dethnode/chain'
import { DethLogger } from './DethLogger'

export const eventLogger = (logger: DethLogger) => (
  runState: InterpreterStep
) => {
  ...
  const [memOffset, memLength, ...topicsRaw] = getLastN(
    runState.stack,
    2 + topicsCount
  )
...
export const revertLogger = (logger: DethLogger) => (
  runState: InterpreterStep
) => {
  const opcodeName = runState.opcode.name
  ...
  const rawDataBuffer = getMemoryAsBuffer(
    runState.memory,
    offset.toNumber(),
    length.toNumber()
  )
