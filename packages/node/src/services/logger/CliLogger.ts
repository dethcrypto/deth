...
import { HexData, Address, makeAddress } from '@dethnode/chain'
...
export class CliLogger implements DethLogger {
  ...
  logTransaction(tx: { to?: Address; from: Address; data?: HexData }) {
    ...
    console.log(
      header('TX'),
      `From ${formatAddress(tx.from)} to ${
        tx.to ? formatAddress(tx.to) : '[NEW CONTRACT]'
  ...
  logEvent(data: string, topics: string[]) {
    ...
    if (decodedLog) {
      ...
      console.log(
        header('LOG'),
        blue(decodedLog.signature),
        stringifyEthersValue(decodedLog.values)
      )
    } else {
  ...
  logRevert(reason: string, address: Address) {
    console.log(
      header('REVERT'),
      red(`Reason ${reason} on ${formatAddress(address)}`)
    )
  }
  ...
  logNodeInfo(walletManager: WalletManager): void {
    ...
    for (const [i, wallet] of wallets.entries()) {
      console.log(
        `${i}\t ${yellow(makeAddress(wallet.address))}\t ${wallet.privateKey}`
      )
    }
    console.log('')
  }
  ...
  logNodeListening(port: number): void {
    ...
    console.log(
      '🌙 Visit the explorer at: ' + blue(`http://localhost:${port}/explorer`)
    )
  }
}
