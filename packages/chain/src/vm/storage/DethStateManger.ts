...
export class DethStateManger {
  constructor(
    private accountsState: CheckpointMap<
      Account,
      Address
    > = new CheckpointMap(),
    private codeState: CheckpointMap<Buffer, Hash> = new CheckpointMap(),
    private storageState: CheckpointMap<
      Dictionary<Buffer>,
      Address
    > = new CheckpointMap(),

    private savedAccountsState: CheckpointMap<
      CheckpointMap<Account>,
      Hash
    > = new CheckpointMap(),
    private savedCodeState: CheckpointMap<
      CheckpointMap<Buffer>,
      Hash
    > = new CheckpointMap(),
    private savedStorageState: CheckpointMap<
      CheckpointMap<Dictionary<Buffer>, Address>,
      Hash
    > = new CheckpointMap(),
    private saveIndex = 0
  ) {}
